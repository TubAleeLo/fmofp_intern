"""
Comprehensive test for all SAR Radar modes in the FMOFP system.
Tests transitions between all modes defined in the sar_radarMode enum.
"""
...
class SARRadarTest:
    """Comprehensive test for all SAR Radar modes and data requests"""
    ...
    def __init__(self):
        ...
        logger.info(f"[TEST::{self.test_id}] SAR Radar test initialized")
    ...
    async def test_all_sar_radar_modes(self):
        """Test ALL SAR Radar modes and transitions between them"""
        try:
            logger.info(f"[TEST::{self.test_id}] === Testing All SAR Radar Modes ===")
            
            # Get all available modes from the enum
            all_modes = list(sar_radarMode)
            logger.info(f"[TEST::{self.test_id}] Testing {len(all_modes)} SAR Radar modes: {[mode.name for mode in all_modes]}")
            ...
            initial_mode = sar_radarMode.STANDBY
            request_id = await self.messages.sar_radar.set_sar_radar_mode(initial_mode)
            ...
            for mode in all_modes:
                ...
                try:
                    request_id = await self.messages.sar_radar.set_sar_radar_mode(mode)
                    ...
                    radar_patterns = [
                        (f"sar_radar.*{mode.name}|setting mode to.*{mode.name}", "Radar mode change confirmation"), 
                        (f"Handling mode change|Processed mode value.*{mode.name}", "Mode change processing")
                    ]
                    
                    # Verify display mode update
                    display_patterns = [
                        (f"display.*sar radar.*{mode.name}", "Display mode update"),
                        (f"sar radar.*mode.*display|display.*mode.*sar radar", "Display processing")
                    ]
                    
                    # Check both radar and display
                    radar_success, radar_missing = self._verify_log_patterns(
                        f"sar_radar_{mode.name}_radar",
                        log_output,
                        radar_patterns
                    )
                    
                    display_success, display_missing = self._verify_log_patterns(
                        f"sar_radar_{mode.name}_display",
                        log_output,
                        display_patterns
                    )
                    ...
                    self._record_test_result(
                        f"sar_radar_to_{mode.name.lower()}_mode",
                        success=success,
                        request_id=request_id,
                        error=error
                    )
                ...
                reset_request_id = await self.messages.sar_radar.set_sar_radar_mode(sar_radarMode.STANDBY)
                await asyncio.sleep(self.MEDIUM_WAIT)
            
            logger.info(f"[TEST::{self.test_id}] === SAR Radar Mode Testing Complete ===")
            
        except Exception as e:
            logger.error(f"[TEST::{self.test_id}] Error in SAR radar mode tests: {str(e)}")
            traceback.print_exc()

    async def test_sar_radar_data_requests(self):
        """Test all SAR Radar data requests"""
        try:
            logger.info(f"[TEST::{self.test_id}] === Testing SAR Radar Data Requests ===")
            
            # Set radar to STRIPMAP mode for data requests (this is one of the main SAR modes)
            logger.info(f"[TEST::{self.test_id}] Setting to STRIPMAP mode for data tests")
            mode_request_id = await self.messages.sar_radar.sar_radar_to_stripmap_mode()
            await asyncio.sleep(self.MEDIUM_WAIT)
            
            # Test imagery data request
            logger.info(f"[TEST::{self.test_id}] Testing imagery data request")
            self.log_capture.seek(0)
            self.log_capture.truncate()
            
            try:
                request_id = await self.messages.sar_radar.request_imagery_data()
                logger.info(f"[TEST::{self.test_id}] Imagery data request sent with ID: {request_id}")
                ...
                log_output = self.log_capture.getvalue()
                patterns = [
                    (r"imagery data|image|sar", "Imagery data processing"),
                ...
                logger.error(f"[TEST::{self.test_id}] Error in imagery data test: {str(e)}")
                self._record_test_result(
                    "request_imagery_data",
                    success=False,
                    error=str(e)
            ...
            # Test resolution data request if available
            try:
                logger.info(f"[TEST::{self.test_id}] Testing resolution data request")
                self.log_capture.seek(0)
                self.log_capture.truncate()
                
                # Use a generic request method or a specific one if available
                if hasattr(self.messages.sar_radar, "request_resolution_data"):
                    request_id = await self.messages.sar_radar.request_resolution_data()
                else:
                    ...
                    request_id = await self.radar_handler.send_request(
                        ...
                        request_type="data",
                        data={"command_type": "resolution_data"}
                    )
                
                logger.info(f"[TEST::{self.test_id}] Resolution data request sent with ID: {request_id}")
                ...
                # Verify logs for resolution data processing
                log_output = self.log_capture.getvalue()
                patterns = [
                    (r"resolution|sar data", "Resolution data processing"),
                    (r"stored|processing", "Data storage or processing")
                ]
                success, missing = self._verify_log_patterns("resolution_data", log_output, patterns)
                
                self._record_test_result(
                    "request_resolution_data",
                    success=success,
                    request_id=request_id,
                    error=None if success else f"Missing log patterns: {', '.join(missing)}"
                )
                ...
                logger.error(f"[TEST::{self.test_id}] Error in resolution data test: {str(e)}")
                self._record_test_result(
                    "request_resolution_data",
                    success=False,
                    error=str(e)
            ...
            logger.info(f"[TEST::{self.test_id}] === SAR Radar Data Testing Complete ===")
            
        except Exception as e:
            logger.error(f"[TEST::{self.test_id}] Error in SAR radar data tests: {str(e)}")
            traceback.print_exc()
    ...
    def generate_test_report(self) -> str:
        ...
        # Create the report
        report = [
            f"==== SAR RADAR TEST RESULTS ({self.test_id}) ====",
        ...
        report_str = "\n".join(report)
        timestamp_report_file = f"sar_radar_test_report_{start.strftime('%Y%m%d_%H%M%S')}.txt"
        json_report_file = f"sar_radar_test_results_{self.test_id}.json"
    ...
    async def run_tests(self):
        """Run all SAR Radar tests"""
        try:
            # Initialize system
            await self.initialize()
            
            # Run tests for all SAR radar modes
            await self.test_all_sar_radar_modes()
            await asyncio.sleep(self.LONG_WAIT)  # Long pause between test groups
            
            # Run tests for SAR radar data requests
            await self.test_sar_radar_data_requests()
