"""
Comprehensive test for all TFR Radar modes in the FMOFP system.
Tests transitions between all modes defined in the tfr_radarMode enum.
"""
...
class TFRRadarTest:
    """Comprehensive test for all TFR Radar modes and data requests"""
    ...
    def __init__(self):
        ...
        logger.info(f"[TEST::{self.test_id}] TFR Radar test initialized")
    ...
    async def test_all_tfr_radar_modes(self):
        """Test ALL TFR Radar modes and transitions between them"""
        try:
            logger.info(f"[TEST::{self.test_id}] === Testing All TFR Radar Modes ===")
            
            # Get all available modes from the enum
            all_modes = list(tfr_radarMode)
            logger.info(f"[TEST::{self.test_id}] Testing {len(all_modes)} TFR Radar modes: {[mode.name for mode in all_modes]}")
            ...
            for mode in all_modes:
                ...
                try:
                    request_id = await self.messages.tfr_radar.set_tfr_radar_mode(mode)
                    ...
                    radar_patterns = [
                        (f"TFR radar.*mode changed|Set TFR radar mode to.*{mode.name}", "Radar mode change confirmation"),
                    ...
                    radar_success, radar_missing = self._verify_log_patterns(
                        f"tfr_radar_{mode.name}_radar",
                        log_output,
                        radar_patterns
                    )
                    
                    display_success, display_missing = self._verify_log_patterns(
                        f"tfr_radar_{mode.name}_display",
                        log_output,
                        display_patterns
                    )
                    ...
                    self._record_test_result(
                        f"tfr_radar_to_{mode.name.lower()}_mode",
                        success=success,
                        request_id=request_id,
                        error=error
                    )
            ...
            # Final reset to STANDBY mode after all mode tests are complete
            logger.info(f"[TEST::{self.test_id}] Final reset to STANDBY mode")
            reset_request_id = await self.messages.tfr_radar.set_tfr_radar_mode(tfr_radarMode.STANDBY)
            await asyncio.sleep(self.MEDIUM_WAIT)
            
            logger.info(f"[TEST::{self.test_id}] === TFR Radar Mode Testing Complete ===")
            
        except Exception as e:
            logger.error(f"[TEST::{self.test_id}] Error in TFR radar mode tests: {str(e)}")
            traceback.print_exc()

    async def test_tfr_radar_data_requests(self):
        """Test all TFR Radar data requests"""
        try:
            logger.info(f"[TEST::{self.test_id}] === Testing TFR Radar Data Requests ===")
            
            # Set radar to ACTIVE mode for data requests (this is the most suitable mode for TFR)
            logger.info(f"[TEST::{self.test_id}] Setting to ACTIVE mode for data tests")
            mode_request_id = await self.messages.tfr_radar.tfr_radar_to_active_mode()
            await asyncio.sleep(self.MEDIUM_WAIT)
            
            # Test elevation data request
            logger.info(f"[TEST::{self.test_id}] Testing elevation data request")
            self.log_capture.seek(0)
            self.log_capture.truncate()
            
            try:
                request_id = await self.messages.tfr_radar.request_elevation_data()
                logger.info(f"[TEST::{self.test_id}] Elevation data request sent with ID: {request_id}")
                ...
                log_output = self.log_capture.getvalue()
                patterns = [
                    (r"elevation data|terrain|tfr", "Elevation data processing"),
                ...
                logger.error(f"[TEST::{self.test_id}] Error in elevation data test: {str(e)}")
                self._record_test_result(
                    "request_elevation_data",
                    success=False,
                    error=str(e)
            ...
            # Test terrain following profile request if available
            try:
                logger.info(f"[TEST::{self.test_id}] Testing terrain following profile request")
                self.log_capture.seek(0)
                self.log_capture.truncate()
                
                # Use a generic request method or a specific one if available
                if hasattr(self.messages.tfr_radar, "request_terrain_following_profile"):
                    request_id = await self.messages.tfr_radar.request_terrain_following_profile()
                else:
                    ...
                    request_id = await self.radar_handler.send_request(
                        ...
                        request_type="data",
                        data={"command_type": "terrain_following_profile"}
                    )
                
                logger.info(f"[TEST::{self.test_id}] Terrain following profile request sent with ID: {request_id}")
                ...
                # Verify logs for terrain following profile processing
                log_output = self.log_capture.getvalue()
                patterns = [
                    (r"terrain following|profile", "Terrain following profile processing"),
                    (r"stored|processing", "Data storage or processing")
                ]
                success, missing = self._verify_log_patterns("terrain_following_profile", log_output, patterns)
                
                self._record_test_result(
                    "request_terrain_following_profile",
                    success=success,
                    request_id=request_id,
                    error=None if success else f"Missing log patterns: {', '.join(missing)}"
                )
                ...
                logger.error(f"[TEST::{self.test_id}] Error in terrain following profile test: {str(e)}")
                self._record_test_result(
                    "request_terrain_following_profile",
                    success=False,
                    error=str(e)
            ...
            logger.info(f"[TEST::{self.test_id}] === TFR Radar Data Testing Complete ===")
            
        except Exception as e:
            logger.error(f"[TEST::{self.test_id}] Error in TFR radar data tests: {str(e)}")
            traceback.print_exc()
    ...
    def generate_test_report(self) -> str:
        ...
        # Create the report
        report = [
            f"==== TFR RADAR TEST RESULTS ({self.test_id}) ====",
        ...
        report_str = "\n".join(report)
        timestamp_report_file = f"tfr_radar_test_report_{start.strftime('%Y%m%d_%H%M%S')}.txt"
        json_report_file = f"tfr_radar_test_results_{self.test_id}.json"
    ...
    async def run_tests(self):
        """Run all TFR Radar tests"""
        try:
            # Initialize system
            await self.initialize()
            
            # Run tests for all TFR radar modes
            await self.test_all_tfr_radar_modes()
            await asyncio.sleep(self.LONG_WAIT)  # Long pause between test groups
            
            # Run tests for TFR radar data requests
            await self.test_tfr_radar_data_requests()
